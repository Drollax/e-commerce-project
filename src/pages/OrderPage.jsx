...
import { setAddress } from '../store/actions/shopActions';

const OrderPage = () => {
  const dispatch = useDispatch();
  
  // 1. Get current address from Redux
  ...
  // Check if an address object exists in Redux and has an ID
  ...
  const deleteAddress = (id) => {
    if (window.confirm("Bu adresi silmek istediğinize emin misiniz?")) {
      API.delete(`/user/address/${id}`)
        .then(() => {
          fetchAddresses();
          // If the deleted address was selected in Redux, clear it
          if (selectedAddress?.id === id) dispatch(setAddress({}));
          toast.info("Adres silindi");
        })
        .catch(err => toast.error("Adres silinemedi"));
    }
    };

  return (
    <div className="max-w-7xl mx-auto p-6 md:p-12 font-montserrat">
      {/* Steps Progress (UI remains consistent) */}
      <div className="flex justify-center mb-10">
        <div className="flex items-center w-full max-w-md">
          <div className="flex flex-col items-center flex-1">
            <div className="w-10 h-10 bg-sky-500 text-white rounded-full flex items-center justify-center font-bold shadow-md">1</div>
            <span className="text-xs mt-2 font-bold text-sky-500">Adres Bilgileri</span>
          </div>
          <div className="h-1 flex-1 bg-stone-200 mx-2"></div>
          <div className="flex flex-col items-center flex-1 opacity-50">
            <div className="w-10 h-10 bg-stone-200 text-stone-500 rounded-full flex items-center justify-center font-bold">2</div>
            <span className="text-xs mt-2 font-medium text-stone-400">Ödeme Seçenekleri</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <OrderSummary 
          buttonText="Kaydet ve Devam Et" 
          onButtonClick={() => toast.info("Ödeme adımına geçiliyor...")}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[500] p-4 backdrop-blur-sm">
          <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-8 rounded-xl w-full max-w-md shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
